...
class SpeechController extends GetxController {
  ...
  Rx<TextEditingController?> selectedField = Rx<TextEditingController?>(null);
  var selectedLocale = 'en_IN'.obs; // Changed to Indian English for better accent detection
  ...
  Future<void> initSpeech() async {
    ...
    if (available.value) {
      Get.snackbar('Info', 'Speak clearly in a quiet environment for best results',
          snackPosition: SnackPosition.BOTTOM);
    }
  }
  ...
  Future<void> startListen() async {
    ...
    await _speech.listen(
      onResult: (result) {
        if (result.finalResult && result.confidence > 0.7) { // Filter low-confidence results
          _updateSelectedField(result.recognizedWords);
        }
      },
      listenMode: stt.ListenMode.dictation,
      pauseFor: const Duration(seconds: 15), // Reduced to avoid noise
      listenFor: const Duration(minutes: 10),
      localeId: selectedLocale.value, // Use dynamic locale
      partialResults: true, // Enable partial results
      cancelOnError: true, // Cancel on errors
      sampleRate: 44100, // High sample rate for clarity
    );
  }
  ...
  // Optional: Allow users to select locale dynamically
